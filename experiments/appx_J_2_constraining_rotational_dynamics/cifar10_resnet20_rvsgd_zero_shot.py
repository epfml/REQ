...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
    ...
    -b ${b} --opt rvwrapper --opt-kwargs "inner_type=${optimizer}" "etar_func=${optimizer}" --momentum 0.9 --lr $lr --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 \
...
weight_decays = [ 0.0001, 0.0001 ]
learning_rates = [ 4.8, 0.5 ]
optimizers = [ "sgdm", "sgdm" ]
bss = [ 2048, 128 ]
