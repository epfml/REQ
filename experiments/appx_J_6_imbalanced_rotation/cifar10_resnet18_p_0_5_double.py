...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
    ...
    -b ${b} --opt $optimizer --opt-kwargs control_group_percentage=0.5 speed=$speed 'is_double=True' --lr $lr --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 \
    --log-wandb --wandb-kwargs project=imbalanced-rotation name="\"rn18_rvc_adamw_b_${b}_iter_${iter}_lr${lr}_wd${wd}_seed${seed}_is_double_f${speed}_sweep\"" --seed $seed \
