...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
    ...
    -b ${b} --opt adamw --lr-base $lr --lr-base-size 128 --lr-base-scale linear --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 0 --min-lr 0 --warmup-epochs 5 --checkpoint-hist 1 \
    --log-wandb --wandb-kwargs project=lr-wd-trade-off name="\"adamw_b_${b}_iter_${iter}_lr${lr}_wd${wd}_seed${seed}\"" --seed $seed \
