...
do
    ...
    do
        ...
        -b 256 --local-accumulation 32 --opt rvwrapper --opt-kwargs "inner_type=sgdm" "etar_func=sgdm" --lr $lr --momentum 0.9 --weight-decay 1e-4 --sched cosine --sched-on-update --epochs 10 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 0 --checkpoint-hist 5 \
        --log-wandb --wandb-kwargs project=lr_warmup name="\"i1k-rn50-nzi-b256x32-bn-rvsgd-lr$lr-s$seed\"" --dynamics-logger-cfg '../../shared/utils/sgd_tracked_logger_cfg_8.yaml' --seed $seed
    done
done
