...
shell_cmd = r"""PYTHONPATH=$PPATH:$base_dir torchrun --nproc_per_node 1 src/main.py \
    --wandb --wandb_project owt3  --wandb_run_prefix gpt_wikitext_slinear_rvadamw_iter_${iter}_lr${lr}_wd${wd}_seed${seed}  \
    --n_embd 768 --n_head 12 --n_layer 12 --batch_size 55 --sequence_length 512 --acc_steps 3 --dropout 0.2 \
    --iterations 15000 --warmup_percent 0.02 --opt rvwrapper --lr $lr --is-test-split \
    --opt-kwargs betas=\(0.9,0.95\) weight_decay=$wd inner_type='adamw' update_norm_decay_factor=0.9 --linear_cfg slinear --seed $seed \
    --scheduler-kwargs div_factor=1e2 final_div_factor=1e4 \
    --dynamics-logger-cfg '../../shared/utils/gpt_rotational_logger_cfg.yaml' \
