...
for index in ${!lrs[*]}; do
    ...
    PYTHONPATH=$PYTHONPATH:$BASE_DIR torchrun --nproc_per_node 1 train.py --output $BASE_DIR/output \
        ...
        --log-wandb --wandb-kwargs project=melo_sv5 name="\"sweep-vgg13-lr$lr-wd$wd-0\"" --log-wandb-extended

    PYTHONPATH=$PYTHONPATH:$BASE_DIR torchrun --nproc_per_node 1 train.py --output $BASE_DIR/output \
        --data-dir $DATA_DIR --dataset torch/CIFAR10 --dataset-download --num-classes 10 --pin-mem --input-size 3 32 32 --mean 0.4914 0.4822 0.4465 --std 0.2023 0.1994 0.2010 --smoothing 0.0 \
        --train-transforms 'RandomCrop=(32,4,)' 'RandomHorizontalFlip=(0.5,)' 'PILToTensor={}' \
        --test-transforms 'PILToTensor={}' \
        --model vgg13 --model-kwargs num_features=512 feature_window_size=1 conv_cfg=wsconv2d linear_cfg=wslinear \
        -b 128 --opt momentum --lr $lr --momentum 0.9 --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 --seed 0 \
        --log-wandb --wandb-kwargs project=melo_sv5 name="\"sweep-vgg13ws-lr$lr-wd$wd-0\"" --log-wandb-extended
done
