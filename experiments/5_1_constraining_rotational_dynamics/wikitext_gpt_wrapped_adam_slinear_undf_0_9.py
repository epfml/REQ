...
shell_cmd = r"""PYTHONPATH=$PPATH:$base_dir torchrun --nproc_per_node 1 src/main.py \
    --wandb --wandb_project adam-wrapper-runs  --wandb_run_prefix llm_slinear_undf_0_9_rvwrapper_iter_${iter}_lr${lr}_wd${wd}_seed${seed}  \
    --n_embd 768 --n_head 12 --n_layer 12 --batch_size 55 --sequence_length 512 --acc_steps 3 --dropout 0.2 \
    --iterations 15000 --warmup_percent 0.02 \
    --opt rvwrapper --opt-kwargs 'inner_type=adam' 'etar_func=adamw' betas=\(0.9,0.95\) 'update_norm_decay_factor=0.9' weight_decay=$wd \
    --lr $lr --is-test-split \
    --linear_cfg slinear --seed $seed \
