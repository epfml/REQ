...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
            ...
            -b ${b} --opt rvwrapper --opt-kwargs 'inner_type=adam' 'etar_func=adamw' --lr $lr --opt-eps 1e-8 --weight-decay $wd --sched cosine --epochs 600 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5  \
            --log-wandb --wandb-kwargs project=adam-wrapper-runs name="\"deit_tiny_patch16_224_rvwrapper_b_${b}_iter_${iter}_lr${lr}_wd${wd}_seed${seed}\"" --seed $seed  \
