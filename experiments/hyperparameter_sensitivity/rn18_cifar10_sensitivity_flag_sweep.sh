...
values1=(1 2 3)
is_zero_mean=("FALSE" "FALSE" "TRUE" )
scale_invariance_type=("channel" "tensor" "channel")
    ...
    do
        ...
        do
            for idx in "${!values1[@]}"; do
                ...
                ch_id=$(($i%1))
                wand_name="cifar_${DNN_MODEL}_c10_${optimizer}_ablation_${scale_invariance}_0mean_${zero_mean}_seed_${seed}_bs_${bath_size}_wd_${weight_decay//./_}_lr_${learning_rate//./_}_run_${i}"
                ...
                " -b $bath_size --opt $optimizer --opt-kwargs zero_mean=$zero_mean scale_invariance=$scale_invariance  --momentum 0.9 --lr-base $learning_rate --lr-base-size 128 --lr-base-scale linear --weight-decay $weight_decay --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1" \
