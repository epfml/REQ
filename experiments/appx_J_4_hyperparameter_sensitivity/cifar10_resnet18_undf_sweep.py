...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
    ...
    -b ${b} --opt $optimizer  --opt-kwargs zero_mean=$zero_mean scale_invariance=$scale_invariance update_norm_decay_factor=$update_norm_decay_factor --momentum 0.9 --lr-base $lr --lr-base-size 128 --lr-base-scale linear --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 \
