...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
    ...
    -b ${b} --opt $optimizer  --opt-kwargs zero_mean=$zero_mean scale_invariance=$scale_invariance --momentum 0.9 --lr-base $lr --lr-base-size 128 --lr-base-scale linear --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 \
    --log-wandb --wandb-kwargs project=cifar-sensitivity-sweep name="\"rvsgd_b_${b}_iter_${iter}_lr${lr}_wd${wd}_seed${seed}_zm${zero_mean}_${scale_invariance}\"" --seed $seed \
...
scale_invariance_types=["channel", "tensor", "channel"]
