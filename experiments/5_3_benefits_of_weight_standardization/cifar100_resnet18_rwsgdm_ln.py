...
-b 256 --opt rvwrapper --lr $lr --weight-decay $wd --momentum 0.9 --opt-kwargs inner_type=sgdm update_norm_decay_factor=$udf --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 \
--log-wandb --wandb-kwargs project=req_iclr_rnln name="\"c100-val-rn18-ln-rwsgdm-udf$udf-lr$lr-s$seed-idx$idx\"" tags="[\"c100_val_rn18_ln_rwsgdm_udf$udf_sweep_lr2\"]" --seed $seed \
...
def main():
    base_dir = os.getcwd()
    seeds = [42, 43, 44]
    lrs = [0.01, 0.02, 0.03, 0.05, 0.1, 0.2, 0.3, 0.5, 1.0, 2.0, 3.0]
    wd = 5e-4
    udf = 0.99
