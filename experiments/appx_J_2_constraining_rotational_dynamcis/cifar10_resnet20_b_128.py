...
shell_cmd = r"""PYTHONPATH=$PYTHONPATH:$base_dir torchrun --nproc_per_node 1 train.py --output $output_dir \
    ...
    -b ${b} --opt $optimizer --momentum 0.9  --lr-base $lr --lr-base-size 128 --lr-base-scale linear --weight-decay $wd --sched cosine --sched-on-update --epochs 200 --warmup-lr 1e-6 --min-lr 1e-5 --warmup-epochs 5 --checkpoint-hist 1 \
    --log-wandb --wandb-kwargs project=constraining-smd name="\"rn20_${optimizer}_b_${b}_iter_${iter}_lr${lr}_wd${wd}_seed${seed}\"" --seed $seed \
    --dynamics-logger-cfg '../../shared/utils/rotational_logger_cfg.yaml' 
"""

weight_decays = [ 0.0001, 1.0, 0.0001, 1.0]
learning_rates = [ 0.5, 0.0005, 0.5, 0.0005]
optimizers = [ "rvsgd", "rvlion", "momentum", "lion"]
...
def main():
    ...
    idx = 0
    for seed in seeds:
        for wd, lr, optimizer in zip(weight_decays, learning_rates, optimizers):
            if do_run(idx):
