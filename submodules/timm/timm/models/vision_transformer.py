...
from shared.modules.factories import get_conv_factory, get_function_dict, get_linear_factory
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = linear_factory(dim, dim * 3, bias=qkv_bias)
    ...
    def forward(self, x):
        ...
        if self.fast_attn:
            ...
            # BMM version of: attn = (q @ k.transpose(-2, -1)) * self.scale
            attn = self.bmm_op(
                q.flatten(0, 1),  # Fold leading dims
                k.transpose(-2, -1).flatten(0, 1)  # Fold leading dims
            ) * self.scale
            attn = attn.view(*q.shape[:2], *attn.shape[1:])  # Unfold leading dims
            # /BMM

            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            # BMM version of: x = (attn @ v)
            x = self.bmm_op(
                attn.flatten(0, 1),  # Fold leading dims
                v.flatten(0, 1),  # Fold leading dims
            ).view(*attn.shape[:3], v.shape[-1])  # Unfold leading dims
            # /BMM
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.linear_factory = get_linear_factory(linear_cfg)
        self.conv_factory = get_conv_factory(conv_cfg)
        self.bmm_op = get_function_dict(bmm_cfg)['bmm']
        ...
        self.head = self.linear_factory(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
