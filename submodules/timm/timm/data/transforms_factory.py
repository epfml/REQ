...
def transforms_imagenet_train(
    ...
    """
    if random_crop_pad:
        # Apply simple RandomCrop (e.g. for CIFAR style ResNet training)
        assert not scale and not ratio, "Mutually exclusive options"
        primary_tfl = [transforms.RandomCrop(img_size, padding=random_crop_pad)]
    else:
        # Apply more complex ImageNet style random resize and crop
...
def create_transform_from_dict(transform_dict):
    # Only supports torchvision transforms for now
