...
def parse_split_numbers(s):
    pattern = r"(\w+)\[(?:(-?\d+(?:\.\d+)?)?:)?(-?\d+(?:\.\d+)?)?\](?:-(\d+))?"
    match = re.search(pattern, s)

    if match:
        word = match.group(1)
        num1 = float(match.group(2)) if match.group(2) is not None else None
        num2 = float(match.group(3)) if match.group(3) is not None else None
        seed = int(match.group(4)) if match.group(4) is not None else None
        return word, num1, num2, seed
    else:
...
def create_dataset(
    ...
    if name.startswith('torch/'):
        ...
        if name in _TORCH_BASIC_DS:
            ...
            use_train = 'train' in split
            ds = ds_class(train=use_train, **torch_kwargs)

            split_name, lower, upper, seed = parse_split_numbers(split)
            if split_name is not None:
                generator = torch.Generator().manual_seed(seed or 0)
                lower = lower or 0.0
                mid = (upper or 1.0) - lower
                upper = 1.0 - (upper or 1.0)
                _, ds, _ = torch.utils.data.random_split(
                    ds,
                    [lower, mid, upper],
                    generator=generator,
                )
