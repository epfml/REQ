...
def save_checkpoint(distributed_backend, model, opt, scheduler, itr, ckpt_dir, name, **extra_args):
    if distributed_backend.is_master_process():
        # Save model and optimizer on master
        checkpoint = dict({
        ...
        torch.save(checkpoint, ckpt_dir / f"{name}.ckpt")

    # Save random states for each worker
    rng_state_dict = {
    ...
    torch.save(rng_state_dict, ckpt_dir / f'{name}_rng_state_{distributed_backend.get_rank()}.ckpt')


def load_checkpoint(distributed_backend, ckpt_dir, name, model=None, opt=None, scheduler=None, load_rng=True):
    ckpt = torch.load(ckpt_dir / f'{name}.ckpt')
    ...
    if load_rng:
        rng_ckpt = torch.load(ckpt_dir / f'{name}_rng_state_{distributed_backend.get_rank()}.ckpt')
        torch.set_rng_state(rng_ckpt['cpu_rng_state'])
        torch.cuda.set_rng_state(rng_ckpt['gpu_rng_state'])
        np.random.set_state(rng_ckpt['numpy_rng_state'])
        random.setstate(rng_ckpt['py_rng_state'])
