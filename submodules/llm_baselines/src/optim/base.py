...
from .utils import eval, get_batch, load_checkpoint, save_checkpoint


def train_base(model, opt, data, scheduler, iterations, acc_steps, batch_size, sequence_length, eval_freq, print_freq, ckpt_dir, distributed_backend, extra_args):
    ...
    if extra_args.resumed:
        print(f"Loading training state from {ckpt_dir}")
    ...
    while itr < iterations:
        ...
        if distributed_backend.is_master_process():
            ...
            if itr % eval_freq == 0 or itr == iterations: # from here it's only evaluation code, all the training is above
                ...
                print_string = f"{epoch}-{itr}/{num_substeps_per_epoch//acc_steps} [train] loss={train_loss:.3f} [val] loss={val_loss:.3f}, pp={val_perplexity:.2f}, acc={val_acc:3f}"
                if scheduler is not None:
                    print_string += f" [lr] {current_lr:.2e}"
                print(print_string)
                ...
                model.train()
            elif itr % print_freq == 0:
                t1 = time.time()
                dt = t1 - t0
                train_loss = loss.detach().cpu().item()
                current_lr = scheduler.get_last_lr()[0] if scheduler is not None else extra_args.lr
                print_string = f"{epoch}-{itr}/{num_substeps_per_epoch//acc_steps} [train] loss={train_loss:.3f}"
                print_string += f" [time per itr] {dt*1000/print_freq:.2f}ms"
                if scheduler is not None:
                    print_string += f" [lr] {current_lr:.2e}"
                print(print_string)
                t0 = time.time()

    print(f"saving checkpoint to {ckpt_dir/'final.ckpt'}")
    save_checkpoint(
        distributed_backend=distributed_backend,
        model=model,
        opt=opt,
