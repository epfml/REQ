...
def get_wikitext_data(is_test_split=False):
    ...
    if not os.path.exists(WIKITEXT_DATA_PATH):
        ...
        with open(os.path.join(WIKITEXT_DATA_PATH, "wikitext-103-raw/wiki.test.raw"), 'r') as data_file:
    ...
    if is_test_split:
        val_data = np.memmap(os.path.join(WIKITEXT_DATA_PATH, 'test.bin'), dtype=np.uint16, mode='r')
