...
class BaseFairseqModel(nn.Module):
    ...
    def get_normalized_probs(
        ...
        return self.get_normalized_probs_scriptable(net_output, log_probs, sample, op_dict=op_dict)
    ...
    def get_normalized_probs_scriptable(
        ...
        if hasattr(self, "decoder"):
            return self.decoder.get_normalized_probs(net_output, log_probs, sample, op_dict=op_dict)
        elif torch.is_tensor(net_output):
            # syntactic sugar for simple models which don't have a decoder
            # (e.g., the classification tutorial)
            logits = net_output.float()
            if log_probs:
                return op_dict['log_softmax'](logits, dim=-1)
            else:
                return op_dict['softmax'](logits, dim=-1)
