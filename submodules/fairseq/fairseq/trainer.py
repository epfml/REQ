...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.op_dict = get_function_dict(self.cfg.optimizer.opt_functional_cfg)
    ...
    def _build_optimizer(self):
        ...
        elif self.cfg.optimization.weight_decay_zero_bias:
            params = []
            for n, p in chain(
                self.model.named_parameters(), self.criterion.named_parameters()
            ):
                if p.requires_grad:
                    if 'embed_tokens.weight' in n or 'bias' in n or torch.squeeze(p).dim() == 1:
                        params.append({'params': p, 'rotational': False, 'weight_decay': 0.0})
                    else:
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("multiply-grads"):
                ...
                self.optimizer.multiply_grads(numer / (sample_size or 1.0), self.op_dict)
            ...
            with torch.autograd.profiler.record_function("clip-grads"):
                # clip grads
                # TODO: PAM this? (not used in default setting)
