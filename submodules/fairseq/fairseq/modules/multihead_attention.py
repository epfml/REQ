...
from shared.modules.factories import get_function_dict, get_linear_factory
...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.using_custom_ops = custom_op_cfg is not None
        linear_factory = get_linear_factory(getattr(custom_op_cfg, 'linear_cfg'))
        op_dict = get_function_dict(getattr(custom_op_cfg, 'functional_cfg'))
        self.bmm_op = op_dict['bmm']
        self.softmax_op = op_dict['softmax']
        self.gain_op = op_dict['mul']
        self.mean_op = op_dict['mean']

        xformers_att_config = utils.eval_str_dict(xformers_att_config)
        self.use_xformers = xformers_att_config is not None
        if self.use_xformers and not _xformers_available:
            raise ImportError("\n\n  Please install xFormers.")
        if self.use_xformers and self.using_custom_ops:
            raise NotImplementedError("No custom op support for xformers")
        ...
        self.k_proj = quant_noise(
            linear_factory(self.kdim, embed_dim, bias=bias), q_noise, qn_block_size
        )
        self.v_proj = quant_noise(
            linear_factory(self.vdim, embed_dim, bias=bias), q_noise, qn_block_size
        )
        self.q_proj = quant_noise(
            linear_factory(embed_dim, embed_dim, bias=bias), q_noise, qn_block_size
        )
    ...
    def forward(
        ...
        if (
            not self.using_custom_ops  # Custom ops not called on this branch
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            assert not self.using_custom_ops, "Not supported here"  # TODO
            ...
            attn_weights = self.bmm_op(q, k.transpose(1, 2))
        ...
        attn_weights_float = utils.softmax(
            attn_weights, dim=-1, onnx_trace=self.onnx_trace, func=self.softmax_op,
        )
        ...
        if need_weights:
            ...
            if not need_head_weights:
                ...
                attn_weights = self.mean_op(attn_weights, dim=0)
