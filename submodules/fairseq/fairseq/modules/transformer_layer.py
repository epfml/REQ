...
from shared.modules.factories import get_function_dict, get_linear_factory, get_norm_factory  # Supermodule import
...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        ...
        self.linear_factory = get_linear_factory(cfg.linear_cfg)
        self.norm_factory = get_norm_factory(cfg.norm_cfg)
        self.op_dict = get_function_dict(cfg.functional_cfg)
        ...
        self.self_attn_layer_norm = LayerNorm(self.embed_dim, export=cfg.export, cls=self.norm_factory)
        self.dropout_module = FairseqDropout(
            cfg.dropout, module_name=self.__class__.__name__
        )
        # TODO: Use PAM activations here (OK for the default ReLU)
        ...
        self.final_layer_norm = LayerNorm(self.embed_dim, export=cfg.export, cls=self.norm_factory)

    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        return quant_noise(
            self.linear_factory(input_dim, output_dim), p=q_noise, block_size=qn_block_size
        )
    ...
    def _prune_fc_layer(self, remove_index: List[int]):
        ...
        self.fc1 = quant_noise(
            self.linear_factory(self.fc1.in_features, self.fc1.out_features - len(remove_index)),
            p=self.quant_noise,
        ...
        self.fc2 = quant_noise(
            self.linear_factory(self.fc2.in_features - len(remove_index), self.fc2.out_features),
            p=self.quant_noise,
            block_size=self.quant_noise_block_size,
        )
...
class TransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.attn_ln = (
            LayerNorm(self.embed_dim, cls=self.norm_factory)
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn_layer_norm = LayerNorm(self.embed_dim, export=cfg.export, cls=self.norm_factory)

        self.ffn_layernorm = (
            LayerNorm(cfg.decoder.ffn_embed_dim, cls=self.norm_factory)
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        return quant_noise(self.linear_factory(input_dim, output_dim), q_noise, qn_block_size)
    ...
    def forward(
        ...
        if self.c_attn is not None:
            ...
            x = self.op_dict['mul'](x, self.c_attn.view(1, 1, -1, 1))
        ...
        if self.w_resid is not None:
            residual = self.op_dict['mul'](self.w_resid, residual)
        x = self.residual_connection(x, residual)
        if not self.normalize_before:
            x = self.final_layer_norm(x)
