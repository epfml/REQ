...
class FairseqAdam(FairseqOptimizer):
    ...
    def optimizer_config(self):
        ...
        return {
            ...
            "opt_functional_cfg": self.cfg.opt_functional_cfg,
        }
...
class Adam(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        weight_decay=0,
        amsgrad=False,
        opt_functional_cfg=None,
    ):
        defaults = dict(
            lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, amsgrad=amsgrad
        )
        self.op_dict = get_function_dict(opt_functional_cfg)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                exp_avg.data = self.op_dict['mul'](exp_avg, beta1) + self.op_dict['mul'](grad, 1 - beta1)
                # exp_avg_sq.mul_(beta2).addcmul_(grad, grad, value=1 - beta2)
                exp_avg_sq.data = self.op_dict['mul'](exp_avg_sq, beta2)
                exp_avg_sq += self.op_dict['mul'](self.op_dict['mul'](grad, grad), 1 - beta2)
                if amsgrad:
                    ...
                    denom = self.op_dict['pow'](exp_avg_sq, 0.5) + group['eps']
                ...
                if group["weight_decay"] != 0:
                    ...
                    # )
                    p_data_fp32 -= self.op_dict['mul'](p_data_fp32, group["weight_decay"] * group["lr"])

                # p_data_fp32.addcdiv_(exp_avg, denom, value=-step_size)
                p_data_fp32 -= self.op_dict['mul'](self.op_dict['div'](exp_avg, denom), step_size)
