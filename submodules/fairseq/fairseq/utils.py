...
def softmax(x, dim: int, onnx_trace: bool = False, func=F.softmax):
    if onnx_trace:
        return func(x.float(), dim=dim)
    else:
        return func(x, dim=dim, dtype=torch.float32)


def log_softmax(x, dim: int, onnx_trace: bool = False, func=F.log_softmax):
    if onnx_trace:
        return func(x.float(), dim=dim)
    else:
        return func(x, dim=dim, dtype=torch.float32)
